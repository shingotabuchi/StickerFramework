// Copy of Unity TextMeshPro/Distance Field shader (TMP_SDF.shader) with an added
// second outline layer rendered outside the first outline. All TMP cginc contents
// are inlined so the shader is self-contained inside the StickerFwk package and
// does not depend on the project-local "Assets/TextMesh Pro/Shaders/" include paths.
Shader "StickerFwk/TextMeshPro/Distance Field Two Outlines" {
...
SubShader {
    ...
    Pass {
        ...
        // ---------- Inlined TMPro_Properties.cginc ----------
        ...
        // Second outline layer (names chosen to match TMP_SDFShaderGUI's
        ...
        fixed4 PixShader(pixel_t input) : SV_Target
        {
            ...
            if (!IsGammaSpace())
            {
                faceColor.rgb = LinearToGammaSpace(faceColor.rgb);
                outlineColor.rgb = LinearToGammaSpace(outlineColor.rgb);
                outline2Color.rgb = LinearToGammaSpace(outline2Color.rgb);
            }
            ...
            bump *= lerp(_BumpFace, _BumpOutline, saturate(sd + outline1 * 0.5));
            n = normalize(n - bump);
            ...
            faceColor.rgb += reflcol.rgb * lerp(_ReflectFaceColor.rgb, _ReflectOutlineColor.rgb, saturate(sd + outline1 * 0.5)) * faceColor.a;
            #endif
